...
17 Claude prompts for running a faceless channel where **AIVDO is your production stack**. Copy-paste into Claude Pro. Every prompt assumes you own the video tool, with no references to Solo.ai / Kling / ElevenLabs retail pricing.

Updated: 2026-04-28. Prompts 4 and 5 revised for V3.1 **Path B** (10-12 min = 2 × AIVDO `standard` jobs, stitched locally). Earlier prompts (2, 3, 6-17) unchanged. Added Channel Voice & Style Constraints section (em-dash ban, anti-AI-filler list, primary-source naming).

---

## Channel Voice & Style Constraints (apply globally to every prompt's output)

These rules apply to ALL text generated by ANY prompt below. They override anything later in this file.

### Punctuation: NEVER use em dashes (—, U+2014) or double-hyphens (--) as punctuation.

Use periods, commas, colons, or parentheses instead. Hyphens in compound words (`founder-failure`, `multi-agent`, `case-study`) are fine.

Replacement patterns:
- "X — Y" (parenthetical aside) → "X. Y" or "X (Y)" or "X, Y"
- "X — and Y" (conjunction) → "X, and Y" or "X. And Y."
- Em dash for emphasis → period instead

Why: the channel owner has a hard style preference against em dashes (saved to memory), and YouTube's description preview also wraps awkwardly around them, making lines look cut off. This rule applies to: narration text (Prompt 4), text_overlays (Prompt 5), titles (Prompts 1, 6), descriptions (Prompt 8), pinned comments, all on-screen captions, and any other generated copy.

### Voice
- Confident analyst tone. No hype. No AI-era filler phrases: "As we delve into...", "It's important to note...", "In today's fast-paced world...", "Let's dive in", "Without further ado", "Buckle up".
- English narration (channel switched from Thai to English on 2026-04-27).
- Active voice over passive.
- Specific over generic: "$1.9B net loss in fiscal 2018" beats "significant losses".
- Cite primary sources by their actual document name (S-1, 10-K, Form 8-K, court docket, earnings transcript) over vague references ("according to reports...", "sources say...").

### Reading-level
- Target: literate adult business reader. Bloomberg Businessweek register, not academic, not BuzzFeed.
- Define jargon on first mention if it's specialty (e.g., "the S-1 is the SEC filing a company submits before going public"). After first mention, use the term naturally.
