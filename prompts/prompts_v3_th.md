...
this file. Most script prompts restate the load-bearing ones inline (register, ค่ะ,
...
ผู้บรรยายคือ **Erinome เสียงหญิง สไตล์ normal (ล็อกไว้แล้วตามสเปก §5.4)** อนุภาคท้ายประโยค
ต้องผูกกับเพศเสียงเสมอ **ปิดท้ายทุกตอนด้วย ค่ะ หรือ นะคะ ห้ามใช้ ครับ เด็ดขาด** แม้แต่ในตัวอย่าง
หรือ draft — นี่คือบั๊กที่เคยเกิดจริงกับเสียง Algieba (ชาย) และเป็นสิ่งที่ `thai_lint.py`
ตรวจจับโดยอัตโนมัติ

### เครื่องหมายวรรคตอนต้องห้าม (Banned punctuation)

**ห้ามใช้ em dash (—) และห้ามใช้ double hyphen (--) ในบทพูดเด็ดขาด** ใช้จุด จุลภาค หรือ
วงเล็บแทน — และห้ามใช้แม้แต่ในตัวอย่างหรือคำอธิบายภายในไฟล์นี้ เพราะโมเดลที่มาอ่านพรอมต์
นี้อาจคัดลอกรูปแบบไปโดยไม่ตั้งใจ นี่คือกฎที่ห้ามคนละอย่างกับข้อจำกัดของ v2 (ซึ่งเป็นความชอบ
ส่วนตัวของเจ้าของช่อง) — กฎยังอยู่เหมือนเดิม เหตุผลเดิม เปลี่ยนแค่ภาษา
...
ต่อให้เสียงสังเคราะห์ดีแค่ไหนก็ตาม จุดสังเกต: อ่านออกเสียงประโยคที่เขียนดัง ๆ — ถ้าไม่มีใครพูด
แบบนี้ในชีวิตจริง ให้เขียนใหม่
...
- **ห้ามบรรยายรูปลักษณ์ทางกายภาพ เครื่องแต่งกาย หรือท่าทางเฉพาะตัวของบุคคลจริงที่มีตัวตน**
  แม้จะไม่เอ่ยชื่อตรงๆ ก็ตาม ถ้าคำบรรยายทำให้จำได้ทันทีว่าเป็นใคร (เช่น "เสื้อคอเต่าสีดำ" —
  Steve Jobs, "เสื้อฮู้ดสีเทา" — Mark Zuckerberg) ก็ยังผิดกฎ silhouette ที่จำได้โดยไม่มีหน้า
...
- **ความหลากหลายของภาพหลักต่อฉาก (anchor-object variety, สืบทอดจาก v2's "hot-dog rule"
  ...
  มือถือ ป้ายบริษัท ถนน) วัตถุเดียวไม่ควรครองความคิดของบทพูดเกินราวๆ 30% ของฉากทั้งหมด — ถ้า
  ทุกฉากพูดถึงวัตถุเดียวกันซ้ำ ระบบสร้างภาพอัตโนมัติมักจะวนภาพซ้ำตาม
...
### CTA ติดตามช่อง (Subscribe CTA — retained from v4.6.3, the one conversion unlock
that empirically held)

ปิดท้ายทุกตอนด้วยประโยคชวนติดตามสั้นๆ เป็นธรรมชาติ ไม่ใช่โฆษณาแข็งๆ ตัวอย่างที่ใช้ได้จริง
(มาจากชุดทดสอบของ `thai_lint.py`, สคริปต์คลีนที่ผ่านทุกกฎ):

> กดติดตาม Disclosed ไว้นะคะ เราปล่อยคลิปใหม่ทุกวันค่ะ

("ทุกวัน" ใช้ได้จริงตามจังหวะรวมของทั้งสองเลน ~1 ตอน/วัน ต่อสัปดาห์ ตาม §5.4 — ถ้าจังหวะ
เปลี่ยนไปในอนาคต ให้แก้คำนี้ ไม่ใช่ตัด CTA ทิ้ง) ห้ามพูดถึง aivdo.ai ในบทพูดของวิดีโอหลัก
(นั่นเป็นงานของคำบรรยายใต้คลิปและ pinned comment ไม่ใช่บทพูด) ห้ามพูดถึง "like button" หรือ
กลไกของ YouTube ตรงๆ ในบทพูด
...
1. ด.ดล Blog (Geek Daily / Geek Story / Geek Talk / Geek Monday) —
...
4. ต่อท้ายด้วย series tag และเลข EP: "<ชื่อตอนภาษาไทย> | {series_tag} EP<เลข>"
   (series_tag คือ "Disclosed Daily" สำหรับ Lane A หรือ "Disclosed Story" สำหรับ Lane B —
   ใช้ตัวเลข EP ของเลนนั้นๆ โดยเฉพาะ ไม่ใช่เลขรวมของทั้งช่อง)
...
- ปิดท้ายด้วย CTA ติดตามช่อง (ดูส่วน Global constraints ด้านบน) ปิดด้วย ค่ะ/นะคะ เท่านั้น
...
เดียวที่ชัดเจน เพราะย่อหน้านี้จะกลายเป็นสัญญาณภาพเดียวที่ระบบสร้างวิดีโอได้รับ — ดู Architecture
note ด้านบนของไฟล์) **ยังไม่ต้องใส่ [Scene N | energy] ตอนนี้** ขั้นตอนนั้นเป็นของ Prompt 4
```
...
**นี่คือ Lane B ("Disclosed Story") — เรื่องที่จบไปแล้ว (postmortem, "ทำไม X ถึงล่ม") หรือ
เรื่องที่คนสงสัยมานาน (curiosity) เน้นหนักไปทางเทคโนโลยีมากกว่าธุรกิจล้วนๆ (spec §5.3 — ไม่ใช่
...
- ทุกฉากปิดท้าย (ฉากสุดท้าย) ด้วย ค่ะ หรือ นะคะ ไม่มี ครับ ที่ไหนเลย
...
3. **ค่ะ/ครับ:** สแกนหาคำว่า "ครับ" ทุกจุด (ต้องไม่มีเลย เพราะผู้บรรยายเป็นเสียงหญิง) และ
   ตรวจว่าฉากสุดท้ายปิดด้วย ค่ะ หรือ นะคะ จริง
