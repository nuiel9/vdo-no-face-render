...
- Voice: ~~Algieba~~ ~~`Erinome`, female~~ **`Sadaltager`, male, `normal` style — LOCKED (re-decided 2026-08-21, Thai spec §5.4/§5.5).** `Algieba` is **male** (`config.py:209`, gender=male, particle=ครับ) — the channel's narrator was male for the entire English run. `Erinome` (female) was picked 2026-08-20 against nine other candidates on descriptors/a short sample; **superseded 2026-08-21** when the owner listened to four candidates against the real EP01 script and picked `Sadaltager` instead ("Knowledgeable" in AIVDO's voice table). Particles flip accordingly: every episode now closes in **ครับ**, not ค่ะ — `thai_lint.py` derives this from `make_request_parts.NARRATOR_GENDER`, a single source of truth, so it doesn't need a manual flip again next time. Gemini TTS stays primary (style control matters for documentary narration); Chirp3-HD stays the fallback it already was — this was never a two-voice question.
...
- **Voice: Gemini TTS, `Sadaltager`, male, `normal` style, locked** (spec §5.4/§5.5, re-decided 2026-08-21 — was `Erinome`, female, chosen 2026-08-20). Thai particles auto-agree to voice gender — every episode closes in ครับ, not ค่ะ.
