...
**Default: `gemini-3.1-flash-image-preview`** ($0.067/image), falling back down AIVDO's existing chain on failure.

Rationale: every video is also an AIVDO demo. Image quality is marketing, not only retention.

### 6.2 Thai text is never generated by an image model

Non-negotiable, and grounded in the project's own findings:

- `aivdo/config.py:153`, dated 2026-06-27: *`gemini-3-pro-image-preview` — renders Thai/CJK text reliably (flash-image garbled it)*.
- Commit `32ad47b` fixed `/omni` by enforcing **no Thai text in generated images** at all.

All Thai on-screen text — titles, callouts, thumbnails, subtitles — is burned in by the renderer with a proper Thai font. `gemini-3-pro-image-preview` is the only chain model that can spell Thai in-frame, and we are not paying for it as a workaround for a typography problem the renderer solves better.
...
| Image model | Per video | Daily (30/mo) |
|---|---|---|
| `imagen-4.0-fast-generate-001` | $1.28–1.60 | $38–48 |
| `gemini-2.5-flash-image` | $2.50–3.12 | $75–94 |
| **`gemini-3.1-flash-image-preview` (chosen)** | **$4.29–5.36** | **$129–161** |
| `gemini-3-pro-image-preview` | $8.58–10.72 | $257–322 |
...
| Images | Gemini only, default `gemini-3.1-flash-image-preview` |
